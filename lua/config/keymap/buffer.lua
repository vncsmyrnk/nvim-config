...
set(
  ...
  "<leader>Y",
  '"+Y',
  { desc = "Copy the rest of the line to system clipboard" }
)
...
set(
  ...
  "<leader>W",
  "<cmd>noa w<cr>",
  { desc = "Save file without triggering autocmds" }
)
...
set(
  "n",
  "<leader>qA",
  session.save_and_force_quit,
  { desc = "Close session without saving" }
)
...
set(
  ...
  "<A-j>",
  ":m '>+1<cr>gv=gv",
  { silent = true, desc = "Move lines down" }
)
...
set(
  ...
  "<A-t>",
  [[<C-\><C-N><C-w>t]],
  { desc = "Go to window on the top left" }
)
set(
  "t",
  "<A-b>",
  [[<C-\><C-N><C-w>b]],
  { desc = "Go to window on the right bottom" }
)
